...
#define CATCH_REGISTER_TAG_ALIAS(alias, spec)                                                                          \
	...
	namespace {                                                                                                    \
	Catch::RegistrarForTagAliases INTERNAL_CATCH_UNIQUE_NAME(AutoRegisterTagAlias)(alias,                          \
										       spec,                           \
...
auto
operator+=(std::string& lhs, StringRef const& sr) -> std::string&;
auto
operator<<(std::ostream& os, StringRef const& sr) -> std::ostream&;
...
void
formatReconstructedExpression(std::ostream& os, std::string const& lhs, StringRef op, std::string const& rhs);
...
void
handleExceptionMatchExpr(AssertionHandler& handler, std::string const& str, StringRef const& matcherString);
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                                         \
	do {                                                                                                           \
		...
		INTERNAL_CATCH_REACT(catchAssertionHandler)                                                            \
		 ...
		 (false) && static_cast<bool>(!!(__VA_ARGS__))) // the expression here is never evaluated at runtime but
...
namespace Detail {
std::string
finalizeDescription(const std::string& desc);
}
...
Approx(std::vector<T> const& comparator)
{
...
[[noreturn]] void
throw_exception(Ex const& e) { throw e; }
...
auto
acquireGeneratorTracker(SourceLineInfo const& lineInfo) -> IGeneratorTracker&;
...
//       return type. Yeah.
auto
generate(SourceLineInfo const& lineInfo, L const& generatorExpression)
...
TestCase
makeTestCase(ITestInvoker* testCase,
...
Detail::CompleteType_t<Detail::ResultOf_t<Fun()>>
user_code(Fun&& fun)
{
...
sample
resample(URng& rng, int resamples, Iterator first, Iterator last, Estimator& estimator)
{
	auto n = last - first;
...
template<typename T>
inline auto
convertInto(std::string const& source, CLARA_CONFIG_OPTIONAL_TYPE<T>& target) -> ParserResult
{
	T temp;
...
[[noreturn]] void
throw_domain_error(std::string const& msg) { throw_exception(std::domain_error(msg)); }
...
void
ReporterRegistry::registerReporter(std::string const& name, IReporterFactoryPtr const& factory)
{
...
void
RunContext::testGroupStarting(std::string const& testSpec, std::size_t groupIndex, std::size_t groupsCount)
{
...
auto
RunContext::acquireGeneratorTracker(SourceLineInfo const& lineInfo) -> IGeneratorTracker&
{
...
void
RunContext::handleUnexpectedExceptionNotThrown(AssertionInfo const& info, AssertionReaction& reaction)
{
...
TestSpec::Matches
TestSpec::matchesByFilter(std::vector<TestCase> const& testCases, IConfig const& config) const
{
...
void
JunitReporter::writeSection(std::string const& className, std::string const& rootName, SectionNode const& sectionNode)
{
